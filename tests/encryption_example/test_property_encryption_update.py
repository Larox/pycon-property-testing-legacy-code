...
from property_testing.encryption_example.encryption_update import EncryptionHandler

load_dotenv()
key = bytes.fromhex(os.getenv("ENCRYPT_KEY_NEW"))
...
encrypted_text = ascii_text.map(
    lambda text: (text, EncryptionHandler(key).encrypt_string(text))
)
